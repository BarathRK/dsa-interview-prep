...
3. Set `previous_end_time` to the end time of the first interval.
4. Traverse the remaining intervals.
5. If the current interval's start time is greater than or equal to `previous_end_time`, update `previous_end_time` to the current interval's end time.
